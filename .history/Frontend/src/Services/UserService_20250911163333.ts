...
class UserService {
  ...
  public constructor() {
    ...
    if (token) {
      ...
      console.log("User from JWT token:", dbUser);
      console.log("User roleId:", dbUser.roleId);
  ...
  public async login(credentials: CredentialsModel): Promise<void> {
    ...
    // Extract user from token:
    const dbUser = jwtDecode<{ user: any }>(token).user;
    
    // Convert role string to roleId number if needed
    if (dbUser.role && !dbUser.roleId) {
      dbUser.roleId = dbUser.role === "Admin" ? 1 : 2;
    }
    
    // Debug: Check what's in the token after login
    console.log("User from login JWT token:", dbUser);
    console.log("User roleId after login:", dbUser.roleId);
