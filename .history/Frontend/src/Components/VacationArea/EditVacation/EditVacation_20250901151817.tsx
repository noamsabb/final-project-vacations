...
useEffect(() => {
    ...
    .then(dbVacation => {
        ...
        setValue("startDate", new Date(dbVacation.startDate).toISOString().split("T")[0]);
        setValue("endDate", new Date(dbVacation.endDate).toISOString().split("T")[0]);
        setValue("price", dbVacation.price);
        setImageUrl(dbVacation.imageUrl!);
        setStartDate(new Date(dbVacation.startDate).toISOString().split("T")[0]);
    })
}, [_id, setValue])
...
<br />
        ...
        <label>Price</label>
        <input type="number" min={0} max={10000} required {...register("price")} />
<br />
