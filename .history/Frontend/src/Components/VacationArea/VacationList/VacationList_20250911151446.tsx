...
export function VacationList() {
  ...
  async function handleFilterChange(selectedFilter: string) {
    ...
    if (likedButton === true) {
       vacations = await userService.getLikedVacationsFiltered(selectedFilter, user._id);
    } else {
    ...
    // { // Calculate the new value BEFORE setting state
    //   const newLikedState = !likedButton;
    //   console.log("Toggling likedButton from", likedButton, "to", newLikedState);
      
    //   // Update state
    //   setLikedButton(newLikedState);
      
    //   // Use the NEW value for logic
    //   if (newLikedState) {
    ...
    //   setFilter(selectedFilter);
    //   setLikedButton(false); // Turn off liked mode
