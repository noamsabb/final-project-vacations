...
class UserController {
  ...
  private async likeVacation(request: Request, response: Response){
    const userId = request.params._id;  // Get user ID from URL
    const vacationId = request.body.vacationId;  // Get vacation ID from body

    await userService.likeVacation(userId, vacationId);  // Pass userId string
