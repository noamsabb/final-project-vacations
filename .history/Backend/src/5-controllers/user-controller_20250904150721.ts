...
class UserController {
  ...
  public constructor() {
    ...
    this.router.post("/api/user/:_id/like",securityMiddleware.verifyToken,this.likeVacation);
    this.router.get("/api/user/:_id/like",securityMiddleware.verifyToken,this.getLikedVacations);
  }
  ...
  private async likeVacation(request: Request, response: Response) {
    ...
    response.status(StatusCode.OK).json({ userId, vacationId });
