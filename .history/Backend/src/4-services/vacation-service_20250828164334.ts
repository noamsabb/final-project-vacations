...
class VacationService {
    ...
    public async getAllVacations(): Promise<IVacationModel[]> {
        // Force proper sorting by using aggregation with explicit date conversion
        const vacations = await VacationModel.aggregate([
            {
                $addFields: {
                    sortDate: { $toDate: "$startDate" }
                }
            },
            {
                $sort: { sortDate: 1 }
            ...
            {
                $project: {
                    sortDate: 0  // Remove the temporary field
                }
            }
        ]).exec();
        
        // Debug logging
        console.log("=== VACATION SORTING DEBUG (FIXED) ===");
