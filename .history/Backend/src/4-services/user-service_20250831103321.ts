...
class UserService {
  ...
  public async likeVacation(userId: string, vacationId: string): Promise<void> {
    ...
    if (isAlreadyLiked) {
      ...
      // Safely decrement likes, but don't go below 0
      await VacationModel.findByIdAndUpdate(
        vacationId, 
        { $max: { likes: { $subtract: [{ $ifNull: ["$likes", 0] }, 1] } } }
      );

    } else {
      const vacation = new mongoose.Types.ObjectId(vacationId);
      user.likedVacations.push(vacation);
      // Safely increment likes, ensuring field exists
      await VacationModel.findByIdAndUpdate(
        vacationId, 
        { $inc: { likes: 1 } },
