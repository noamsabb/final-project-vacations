...
class UserService {
    ...
    public async likeVacation(userId: string, vacationId: string): Promise<void>{
        ...
        const vacationObjectId = new mongoose.Types.ObjectId(vacationId);

        // Check if vacation is already liked
        const isAlreadyLiked = user.likedVacations.some(id => id.toString() === vacationObjectId.toString());
        
        if (isAlreadyLiked) {
            // Remove from liked vacations (unlike)
            user.likedVacations = user.likedVacations.filter(id => id.toString() !== vacationObjectId.toString());
        } else {
