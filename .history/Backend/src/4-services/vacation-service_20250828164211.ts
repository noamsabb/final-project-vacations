...
class VacationService {
    ...
    public async getAllVacations(): Promise<IVacationModel[]> {
        ...
        // Debug logging with raw date values
        console.log("=== VACATION SORTING DEBUG ===");
        console.log(`Found ${vacations.length} vacations`);
        vacations.forEach((vacation, index) => {
            console.log(`${index + 1}. ${vacation.destination}`);
            console.log(`   Raw startDate: ${vacation.startDate}`);
            console.log(`   Date object: ${new Date(vacation.startDate)}`);
            console.log(`   Timestamp: ${new Date(vacation.startDate).getTime()}`);
            console.log(`   ---`);
        });
        console.log("=============================");
