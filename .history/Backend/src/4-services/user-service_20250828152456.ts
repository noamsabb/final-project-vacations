...
// Interface for user with populated liked vacations
interface IUserWithLikedVacations extends Omit<IUserModel, 'likedVacations'> {
  likedVacations: IVacationModel[];
}
...
class UserService {
  ...
  public async getLikedVacations(userId: string): Promise<IVacationModel[]>{
    ...
    // When populated, likedVacations becomes an array of vacation documents
    return user.likedVacations as unknown as IVacationModel[];
  }
}
